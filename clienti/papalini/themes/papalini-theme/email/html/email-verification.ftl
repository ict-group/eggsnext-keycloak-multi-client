      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:620px;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #dfe6f1;box-shadow:0 10px 28px rgba(37,52,102,0.14);">
        ...
        <tr>
          <td style="padding:34px 36px 22px 36px;">
            <h1 style="margin:0 0 14px 0;font-size:25px;line-height:1.25;color:#253466;font-weight:700;">${msg("verifyTitle")}</h1>
            <p style="margin:0 0 14px 0;font-size:16px;line-height:1.6;color:#3d4668;">Ciao,</p>
            <p style="margin:0 0 14px 0;font-size:16px;line-height:1.6;color:#3d4668;">${msg("verifyIntro")} <strong style="color:#253466;">${realmName!msg("companyName")}</strong>.</p>
            <p style="margin:0 0 22px 0;font-size:16px;line-height:1.6;color:#3d4668;">${msg("verifyHelp")}</p>
            <table role="presentation" cellpadding="0" cellspacing="0" style="margin:28px 0 26px 0;">
              <tr><td align="center" bgcolor="#253466" style="border-radius:10px;">
                <a href="${link!''}" style="display:inline-block;padding:14px 28px;font-size:15px;font-weight:700;line-height:1.2;color:#ffffff;text-decoration:none;border-radius:10px;background:#253466;text-transform:uppercase;letter-spacing:.04em;">${msg("verifyButton")}</a>
              </td></tr>
            </table>
            <p style="margin:0 0 12px 0;font-size:14px;line-height:1.6;color:#5a6383;">${msg("verifyExpiration")} <strong>${expirationText}</strong>.</p>
            <p style="margin:0 0 18px 0;font-size:14px;line-height:1.6;color:#5a6383;">${msg("verifyIgnore")}</p>
            <div style="margin:24px 0 0 0;padding:16px;background:#f4f7fb;border-radius:12px;border:1px solid #dfe6f1;">
              <p style="margin:0 0 8px 0;font-size:13px;line-height:1.5;color:#68718f;">${msg("verifyFallback")}</p>
